...
func (eventSelectSqlManager) ByCodes(numberOfEvents int) string {
	...
	return fmt.Sprintf(`SELECT id AS event_id, code AS event_code, title AS event_title,
				...
				COALESCE(ends_at, '0001-01-01 00:00:00') AS event_ends_at, location AS event_location,
...
func (eventSelectSqlManager) OccurringToday() string {
	return `SELECT event.id AS event_id, event.code AS event_code, event.title AS event_title,
				...
				COALESCE(event.ends_at, '0001-01-01 00:00:00') AS event_ends_at, event.location AS event_location,
			...
			WHERE event.active = true AND event_situation.active = true AND
				DATE_TRUNC('day', event.starts_at) <= CURRENT_DATE AND
				DATE_TRUNC('day', COALESCE(event.ends_at, event.starts_at)) >= CURRENT_DATE
